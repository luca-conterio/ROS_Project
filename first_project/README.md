...
In the `msg` folder two custom message definitions are contained:
  * `floatStamped.msg`: used to subscribe to car sensors data, since bag files data is formatted as this message type.
...
For what concerns changing the odometry computation type, it can be done simple changing the `odometry_type` parameter (dropdown menu in `rqt_reconfigure`), selecting the desired odometry type.
